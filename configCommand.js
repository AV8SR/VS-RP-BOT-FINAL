const { Client, GatewayIntentBits, EmbedBuilder, MessageFlags, SelectMenuBuilder, ActionRowBuilder, SlashCommandBuilder, PermissionsBitField, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ButtonBuilder, ButtonStyle } = require('discord.js');
...
const createMainMenu = () => {
   const configEmbed = new EmbedBuilder()
      .setTitle('Ticketing Configurator')
      .setDescription('Use the select menu below to configure a specific component.')
      .setFooter({ text: 'This select menu will expire in 5 minutes.'})
   ...
   const selectMenu = new StringSelectMenuBuilder()
      ...
      .addOptions(
         new StringSelectMenuOptionBuilder()
            .setLabel('Resend Embed')
            .setDescription('Resend the ticketing embed.')
            .setValue('resend_embed')
            .setEmoji('📨'),
         new StringSelectMenuOptionBuilder()
            .setLabel('Manage Administrators')
            .setDescription('Add/remove administrators from the ticketing system.')
            .setValue('manage_administrators')
            .setEmoji('🔑'),
         new StringSelectMenuOptionBuilder()
            .setLabel('Emojis')
            .setDescription('Configure the emojis used in command responses.')
            .setValue('emojis')
            .setEmoji('😀'),
         new StringSelectMenuOptionBuilder()
            .setLabel('Ticket Categories')
            .setDescription('Configure the ticket categories where the tickets are made.')
            .setValue('ticket_categories')
            .setEmoji('📠'),
         new StringSelectMenuOptionBuilder()
            .setLabel('Ticket Channel')
            .setDescription('Configure the channel where the ticket embed is sent.')
            .setValue('ticket_channel')
            .setEmoji('📥'),
         new StringSelectMenuOptionBuilder()
            .setLabel('Ticket Transcripts')
            .setDescription('Configure the channel and format of the ticket transcripts.')
            .setValue('ticket_transcripts')
            .setEmoji('📑'),
         new StringSelectMenuOptionBuilder()
            .setLabel('Ticket Roles')
            .setDescription('Configure the people who can manage and respond to tickets (role-based).')
            .setValue('ticket_roles')
            .setEmoji('👥'),
         new StringSelectMenuOptionBuilder()
            .setLabel('Ticket Statuses')
            .setDescription('Configure the status for each ticket type.')
            .setValue('ticket_statuses')
            .setEmoji('🔒')
      )
...
const createEmojiMenu = () => {
   ...
   const primaryActionRow = new ActionRowBuilder()
      .addComponents((new StringSelectMenuBuilder()
         .setCustomId('emoji_select_change')
         .setPlaceholder('Choose the emoji to change.')
         .setMinValues(1)
         .setMaxValues(1)
         .addOptions(
            new StringSelectMenuOptionBuilder()
               .setLabel('Success')
               .setDescription('Change the success emoji.')
               .setValue('success')
               .setEmoji(checkEmoji),
            new StringSelectMenuOptionBuilder()
               .setLabel('Error')
               .setDescription('Change the error emoji.')
               .setValue('error')
               .setEmoji(errorEmoji),
            new StringSelectMenuOptionBuilder()
               .setLabel('Loading')
               .setDescription('Change the loading emoji.')
               .setValue('loading')
               .setEmoji(loadingEmoji),
            new StringSelectMenuOptionBuilder()
               .setLabel('Reset')
               .setDescription('Reset emojis to the defaults.')
               .setValue('reset')
               .setEmoji(resetEmoji),
         )
      ));
   ...
   const actionRow = new ActionRowBuilder()
      .addComponents(back, confirm, cancel);
   ...
   try {
      while (true) {
         ...
         if (collector.values[0] === 'emojis') {
            ...
            const jsondata = JSON.parse(fs.readFileSync('./orders/kae-ticketing/tickets.json', 'utf8'));
            const bannerEmbed = new discord.EmbedBuilder()
               .setColor('Grey')
               .setImage('https://i.imgur.com/MTDRpyV.png')

            const embed = new discord.EmbedBuilder()
               .setTitle('Support Center')
               .setDescription('Use the select menu to choose the appropriate type of ticket to enter. Some information to guide you can be found below.')
               .addFields(
                  { name: '<:kaesupport:1373578230535028756> General Support', value: '- General inquiries\n- Rule clarifications\n- Race descriptions\n- Suggestions'},
                  { name: '<:kaeinternal:1373578216379383829> Internal Affairs', value: '- Staff complaints\n- Member complaints\n- Appealing infractions'},
                  { name: '<:kaedirective:1373578245236064276> Directive', value: '- Internal Affairs+ complaints\n- Internal Affairs+ appeals\n- Claiming prizes\n- Partnership Request'}
               )
               .setColor('Grey')
               .setFooter({ text: 'Support Center' })
               .setImage('https://i.imgur.com/Zz9x9jl.png')
            
            const ticketTypeSelect = new discord.StringSelectMenuBuilder()
               ...
               .addOptions(
                  new discord.StringSelectMenuOptionBuilder()
                     ...
                     .setEmoji('<:kaesupport:1373578230535028756>'),
                  new discord.StringSelectMenuOptionBuilder()
                     .setLabel('Internal Affairs')
                     .setDescription('Staff complaints.')
                     .setValue('ticket-internalAffairs')
                     .setEmoji('<:kaeinternal:1373578216379383829>'),
                  new discord.StringSelectMenuOptionBuilder()
                     .setLabel('Directive')
                     .setDescription('Contacting the directive team.')
                     .setValue('ticket-directive')
                     .setEmoji('<:kaedirective:1373578245236064276>')
            );
            
            const actionRow = new discord.ActionRowBuilder()
               .addComponents(ticketTypeSelect);
            ...
            const confirm = new ButtonBuilder()
               .setCustomId('manage_administrators_confirm')
               .setLabel('✔')
               .setStyle(ButtonStyle.Success)
            ...
            const cancel = new ButtonBuilder()
               .setCustomId('manage_administrators_cancel')
               .setLabel('✖')
               .setStyle(ButtonStyle.Danger)
            ...
            const adminSelect = new discord.UserSelectMenuBuilder()
               .setCustomId('manage_administrators_select')
               .setPlaceholder('Edit the list of administrators.')
               .setMinValues(0)
               .setDefaultUsers(currAdmins)
               .setMaxValues(10);
            
            const adminActionRow = new ActionRowBuilder()
               .addComponents(adminSelect);
            ...
            while (true) {
               ...
               if (newCollector.customId === 'manage_administrators_confirm') {
                  ...
                  fs.writeFileSync('./orders/kae-ticketing/tickets.json', JSON.stringify(jsondata, null, 2));
            ...
            const confirm = new ButtonBuilder()
               .setCustomId('ticket_categories_confirm')
               .setLabel('✔')
               .setStyle(ButtonStyle.Success)
            ...
            const cancel = new ButtonBuilder()
               .setCustomId('ticket_categories_cancel')
               .setLabel('✖')
               .setStyle(ButtonStyle.Danger)
            ...
            const categorySelect = new discord.ChannelSelectMenuBuilder()
               .setCustomId('ticket_categories_select')
            ...
            while (true) {
               ...
               if (newCollector.customId === 'ticket_categories_confirm') {
                  ...
                  if (newCategoryEmbed.data.fields[1].value === 'No changes have been made yet.') { newCategoryEmbed.data.fields[1].value = `<#${currCategory}> ⟶ <#${categorySelect}>`; } else { newCategoryEmbed.data.fields[1].value += `\n<#${categoryChanges[categoryChanges.length - 2].value}> ⟶ <#${categorySelect}>`; }
            ...
            const confirm = new ButtonBuilder()
               .setCustomId('ticket_channel_confirm')
               .setLabel('✔')
               .setStyle(ButtonStyle.Success)
            ...
            const cancel = new ButtonBuilder()
               .setCustomId('ticket_channel_cancel')
               .setLabel('✖')
               .setStyle(ButtonStyle.Danger)
            ...
            while (true) {
               ...
               if (newCollector.customId === 'ticket_channel_confirm') {
                  ...
                  if (newChannelEmbed.data.fields[1].value === 'No changes have been made yet.') { newChannelEmbed.data.fields[1].value = `<#${currChannel}> ⟶ <#${channelSelect}>`; } else { newChannelEmbed.data.fields[1].value += `\n<#${channelChanges[channelChanges.length - 2].value}> ⟶ <#${channelSelect}>`; }
            ...
            const confirm = new ButtonBuilder()
               .setCustomId('ticket_transcripts_confirm')
               .setLabel('✔')
               .setStyle(ButtonStyle.Success)
            ...
            const cancel = new ButtonBuilder()
               .setCustomId('ticket_transcripts_cancel')
               .setLabel('✖')
               .setStyle(ButtonStyle.Danger)
            ...
            const transcriptTypeSelect = new StringSelectMenuBuilder()
               .setCustomId('ticket_transcripts_select')
               .setPlaceholder('Select a transcript type.')
               .setMinValues(1)
               .setMaxValues(1)
               .addOptions(
                  new StringSelectMenuOptionBuilder()
                     .setLabel('Text File')
                     .setDescription('A text file will be sent to the channel.')
                     .setValue('text_file')
                     .setEmoji('📄'),
                  new StringSelectMenuOptionBuilder()
                     .setLabel('Website')
                     .setDescription('HTML code for a website will be uploaded to github and available to visit at a URL.')
                     .setValue('website')
                     .setEmoji('🌐'),
                  new StringSelectMenuOptionBuilder()
                     .setLabel('None')
                     .setDescription('No transcripts will be made.')
                     .setValue('none')
                     .setEmoji('❌'),
               )

            const transcriptTypeActionRow = new ActionRowBuilder()
               .addComponents(transcriptTypeSelect);
            ...
            while (true) {
               ...
               if (newCollector.customId === 'ticket_transcripts_confirm') {
                  ...
                  if (newTranscriptTypeEmbed.data.fields[1].value === 'No changes have been made yet.') { newTranscriptTypeEmbed.data.fields[1].value = `${currTranscriptType} ⟶ ${transcriptTypeSelect}`; } else { newTranscriptTypeEmbed.data.fields[1].value += `\n${currTranscriptType} ⟶ ${transcriptTypeSelect}`; }
            ...
            const selectRoleGroup = new discord.StringSelectMenuBuilder()
               .setCustomId('ticket_roles_select')
               .setPlaceholder('Select the role group to configure.')
               .setMinValues(1)
               .setMaxValues(1)
               .addOptions(
                  new discord.StringSelectMenuOptionBuilder()
                     .setLabel('Support')
                     .setDescription('Roles to respond to support tickets.')
                     .setValue('support')
                     .setEmoji('<:kaesupport:1373578230535028756>'),
                  new discord.StringSelectMenuOptionBuilder()
                     .setLabel('Internal Affairs')
                     .setDescription('Roles to respond to internal affairs tickets.')
                     .setValue('internalAffairs')
                     .setEmoji('<:kaeinternal:1373578216379383829>'),
                  new discord.StringSelectMenuOptionBuilder()
                     .setLabel('Directive')
                     .setDescription('Roles to respond to directive tickets.')
                     .setValue('directive')
                     .setEmoji('<:kaedirective:1373578245236064276>'),
               )
            ...
            const selectRoleActionRow = new ActionRowBuilder()
               .addComponents(selectRoleGroup);
            ...
            while (true) {
               ...
               if (groupTypeCollector.customId === 'ticket_roles_select') {
                  ...
                  if (listOfCurrRoles && listOfCurrRoles.length > 0) {
                     removeRolesSelect = new discord.RoleSelectMenuBuilder()
                        .setCustomId('ticket_roles_remove')
                        .setPlaceholder('Select the roles to remove.')
                        .setMinValues(1)
                        .setMaxValues(listOfCurrRoles.length)
                  ...
                  const confirm = new ButtonBuilder()
                     .setCustomId('confirm_role_changes')
                     .setLabel('✔')
                     .setStyle(ButtonStyle.Success)
                  
                  const cancel = new ButtonBuilder()
                     .setCustomId('cancel_role_changes')
                     .setLabel('✖')
                     .setStyle(ButtonStyle.Danger)
                  
                  const addSelectRow = new ActionRowBuilder()
                     .addComponents(addRolesSelect);

                  const removeSelectRow = new ActionRowBuilder()
                     .addComponents(removeRolesSelect);
            ...
            const selectStatusGroup = new discord.StringSelectMenuBuilder()
               .setCustomId('ticket_statuses_select')
               .setPlaceholder('Select the status group to configure.')
               .setMinValues(1)
               .setMaxValues(1)
               .addOptions(
                  new discord.StringSelectMenuOptionBuilder()
                     .setLabel('Support')
                     .setDescription('Toggle the support ticket status.')
                     .setValue('support')
                     .setEmoji('<:kaesupport:1373578230535028756>'),
                  new discord.StringSelectMenuOptionBuilder()
                     .setLabel('Internal Affairs')
                     .setDescription('Toggle the internal affairs ticket status.')
                     .setValue('internalAffairs')
                     .setEmoji('<:kaeinternal:1373578216379383829>'),
                  new discord.StringSelectMenuOptionBuilder()
                     .setLabel('Directive')
                     .setDescription('Toggle the directive ticket status.')
                     .setValue('directive')
                     .setEmoji('<:kaedirective:1373578245236064276>'),
               )
            ...
            const confirm = new ButtonBuilder()
               .setCustomId('confirm_status_changes')
               .setLabel('✔')
               .setStyle(ButtonStyle.Success)

            const cancel = new ButtonBuilder()
               .setCustomId('cancel_status_changes')
               .setLabel('✖')
               .setStyle(ButtonStyle.Danger)
            ...
            const statusSelectRow = new ActionRowBuilder()
               .addComponents(selectStatusGroup);
      ...
      while (true) {
         ...
         if (newCollector.customId === 'emojis_confirm') {
            ...
            const modal = new discord.ModalBuilder()
               .setCustomId('emoji_change')
               .setTitle(`${emojiChangeChoice} Emoji Change`);

            const emojiInput = new discord.TextInputBuilder()
               .setCustomId('emoji_input')
               .setLabel('New emoji:')
               .setPlaceholder('Symbol or discord emoji (e.g. <:emoji:12345>)')
            ...
            try {
               ...
               if (emojisEmbed.data.fields[0].value === 'No changes have been made yet.') { emojisEmbed.data.fields[0].value = ''; }
